...
listParams = {
    ...
    "svm_poly": listP({"C": [0.1, 1, 10], "d": [2, 3, 4]}),
}
...
for dataset in ["glass", "wine"]:
    ...
    for algo in listParams.keys():  # on parcourt l'ensemble des algorithmes
        if len(listParams[algo]) > 1:  # Début Cross-Validation
            validParam = (
                []
            )  # Stockage du score moyen par éléments de notre grille hyper-paramètres
            for param in listParams[
                algo
            ]:  # On parcourt l'ensemble des hyper-paramètres
                valid = []  # stockage des scores pour sur chaque fold
                for iFoldVal in range(nbFoldValid):
                    fTrain, fValid = foldsTrainValid[
                        iFoldVal
                    ]  # définition du fold de validation
                    # Normalisation
